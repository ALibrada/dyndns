 ...
 ##############################################################################
 DNSFolder=$HOME/dns
 UpdateAddress=$(cat $DNSFolder/update.conf)
 if test -f $DNSFolder/dnsactual.conf
   then
   CacheIP=$(cat $DNSFolder/dnsactual.conf)
 fi
 #echo $CacheIP
 ...
 then
   ...
   wget $UpdateAddress -o /dev/null -O /dev/stdout
   echo `date`  "Updating log with IP " $CurreIP >> dnsactual.log
 fi
 rm -f $DNSFolder/dnsactual.conf
 echo $CurreIP > $DNSFolder/dnsactual.conf
